...
class AccessibilityDialog extends ConsumerWidget {
  ...
  Widget build(BuildContext context, WidgetRef ref) {
    ...
    Color primaryColor =
        hc ? const Color(0xFF9FA8DA) : _getColorForRole(provider.currentRole);
    ...
    return Container(
      ...
      child: Column(
        ...
        crossAxisAlignment:
            CrossAxisAlignment.start, // Changed to start (Right in RTL)
        children: [
          Center(
              child: Container(
                  width: 40,
                  height: 4,
                  decoration: BoxDecoration(
                      color: hc ? Colors.white12 : const Color(0xFFe2e8f0),
                      borderRadius: BorderRadius.circular(10)))),
          const SizedBox(height: 24),
          Text('إعدادات الرؤية والخط',
              style: TextStyle(
                  fontSize: 24,
                  fontWeight: FontWeight.bold,
                  color: hc ? Colors.white : const Color(0xFF1e293b))),
          const SizedBox(height: 8),
          Text('تحكم في حجم الكلام ليكون مريحاً لعينك',
              style: TextStyle(
                  fontSize: 16,
  ...
  Widget _buildScaleControl(
      AppRiverpod provider, WidgetRef ref, Color primaryColor) {
    bool hc = provider.isHighContrast;
    bool dm = provider.isDarkMode;
    return Column(
      crossAxisAlignment:
      ...
      children: [
        Text('حجم الخط',
            style: TextStyle(
                fontSize: 18,
                fontWeight: FontWeight.bold,
                color: (hc || dm) ? Colors.white : const Color(0xFF1e293b))),
        const SizedBox(height: 16),
        Row(
          children: [
            Icon(Icons.text_fields_rounded,
                size: 20,
                color: (hc || dm) ? Colors.white24 : const Color(0xFF94a3b8)),
            Expanded(
              child: Slider(
                ...
                divisions: 7,
                activeColor:
                    (hc || dm) ? const Color(0xFF9FA8DA) : primaryColor,
                inactiveColor:
                    (hc || dm) ? Colors.white12 : const Color(0xFFe2e8f0),
                onChanged: (val) => ref.read(appRiverpod).updateFontScale(val),
              ),
            ),
            Icon(Icons.text_fields_rounded,
                size: 32,
                color: (hc || dm) ? const Color(0xFF9FA8DA) : primaryColor),
          ],
        ),
        Center(
          child: Container(
            ...
            children: [
              Text('الوضع الليلي (Night Mode)',
                  style: TextStyle(
                      ...
                      fontWeight: FontWeight.bold,
                      color:
                          (hc || dm) ? Colors.white : const Color(0xFF1e293b))),
              Text('واجهة داكنة مريحة للعين',
                  style: TextStyle(
                      fontSize: 12,
                      color: (hc || dm)
                          ? Colors.white70
      ...
      children: [
        ...
        Expanded(
          child: FittedBox(
            ...
            child: Text('وضع التباين العالي (لضعف الرؤية)',
                style: TextStyle(
                    fontSize: 18,
                    fontWeight: FontWeight.bold,
                    color:
